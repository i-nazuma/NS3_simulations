...
main(int argc, char* argv[])
{
    ...
    uint8_t numBands = 2;
    double centralFrequencyBand40 = 2350e6; // default is 2350e6
    double bandwidthBand40 = 50e6;
    double centralFrequencyBand38 = 2595e6; // default is 2595e6
    ...
    double totalTxPower = 17; //default is 13
    std::string pattern =
   ...
   // int maxPackets = 28500;
    ...
    double gNbHeight = 10;
    double ueHeight = 9.5; //default is 1.5

    gNbNodes.Create(numNodePairs); // 3 GNB
    ueNodes.Create(numNodePairs);  // 4 UE
    ...
    // mobility.Install(ueNodes);
    // std::string traceFile = "/home/nadeem/5G-NS3/ns-3-dev/scratch/ns2mobility.ns_movements";
    ...
    for (int i = 0; i < numNodePairs; i++)
    {
        gNbNodes.Get(i)->GetObject<MobilityModel>()->SetPosition(Vector(xVal, 800, 0));
        gNbNodes.Get(i)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(45, 0, 0));
        xVal += 30;
    }
    /*ueNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(50, 800, 0));
    ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(120, 0, 0));

    gNbNodes.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(90, 800, 0));
    gNbNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(60, 0, 0));
    */
    ...
    ulClientVoice.SetAttribute("MaxPackets", UintegerValue(1000000));
    ulClientVoice.SetAttribute("PacketSize", UintegerValue(udpPacketSizeBe));
    ulClientVoice.SetAttribute("Interval", TimeValue(interPacketInterval));
    ...
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i)
    {
        ...
        if (enableGaming) // I myself make it false
        {
    ...
    ns3::AnimationInterface anim("5G-Animation.xml");
    Simulator::Run();
    ...
    */
         ...
         ++i)
    {
        ...
        outFile << "Flow " << i->first << " (" << t.sourceAddress << ":" << t.sourcePort << " -> "
                ...
                << protoStream.str() << "\n";
        outFile << "All Packets  Transmitted: " << i->second.txPackets << "\n";
        outFile << "All Packets Received    :" << i->second.rxPackets << "\n";
        outFile << "All Drop Packets        :" << i->second.txPackets - i->second.rxPackets << "\n";
        pkt_sent = i->second.txPackets;
        pkt_recvd = i->second.rxPackets;
        pkt_dropped = i->second.txPackets - i->second.rxPackets;
        // outFile << "  Tx Bytes:   " << i->second.txBytes << "\n";
        // outFile << "  TxOffered:  "
        //       << i->second.txBytes * 8.0 / (simTime - udpAppStartTime) / 1000 / 1000 << "
        //       Mbps\n";
        ...
        if (i->second.rxPackets > 0)
        {
            ...
            //       << " Mbps\n";
            outFile << "Delay in milli second   :"
                    << 1000 * i->second.delaySum.GetSeconds() / i->second.rxPackets << " ms\n";
            // outFile << "  Mean upt:  " << i->second.uptSum / i->second.rxPackets / 1000/1000 << "
            // Mbps \n";
            outFile << "Jitter in millisecond   :"
                    << 1000 * i->second.jitterSum.GetSeconds() / i->second.rxPackets << " ms\n";
            outFile << "Packets Delivery Ratio  :" << (pkt_recvd / pkt_sent) * 100 << "%\n";
            outFile << "Packets Lost Ratio      :" << (pkt_dropped / pkt_sent) * 100 << "%\n";
        }
        else
        {
    ...
    std::map<FlowId, FlowMonitor::FlowStats> stats1 = flowMonitor->GetFlowStats();
    uint64_t txPacketsum = 0;
    uint64_t rxPacketsum = 0;
    uint64_t DropPacketsum = 0;
    double Jitter = 0;
    double Delaysum = 0;
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats1.begin();
         i != stats1.end();
         ++i)
    {
    ...
    std::cout << "\nDelay in milli second   :  " << Delaysum / txPacketsum;
    std::cout << "\nJitter in millisecond   :  " << Jitter / txPacketsum;
